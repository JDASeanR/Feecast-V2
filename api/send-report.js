...
module.exports = async (req, res) => {
  ...
  try {
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:'Helvetica Neue',sans-serif;max-width:560px;margin:0 auto;color:#3D3935">
          <div style="background:#3D3935;padding:20px 24px;border-radius:6px 6px 0 0">
            <div style="font-size:18px;font-weight:700;color:#F5F5F1;letter-spacing:0.04em;text-transform:uppercase">${reportName}</div>
            <div style="font-size:11px;color:rgba(245,245,241,0.5);margin-top:4px">JD+A Feecast · ${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div>
          </div>
          <div style="background:#F5F5F1;padding:20px 24px;border:1px solid rgba(61,57,53,0.1);border-top:none;border-radius:0 0 6px 6px">
            ...
            <p style="margin:0;font-size:13px;color:#736F4C">The report is attached as a PDF. Generated from live Feecast data.</p>
          </div>
        </div>
      `,
      attachments: [{
