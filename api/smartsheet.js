...
export default async function handler(req, res) {
  ...
  if (!token || !sheetId) {
    return res.status(400).json({ error: 'Missing token or sheetId', got: { token: !!token, sheetId: !!sheetId } });
  }

  // Clean the sheetId — strip any whitespace
  const cleanSheetId = String(sheetId).trim();
  const url = `https://api.smartsheet.com/2.0/sheets/${cleanSheetId}`;
